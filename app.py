...
def word_cloud(wd_list, filename):
    ...
    plt.savefig(f'static/images/{filename}_wordcloud.png',
...
def tweet_daily(dataset, filename):
    dataset.Timestamp = pd.to_datetime(
        dataset['Timestamp']).dt.strftime('%d/%m/%Y')
    group = dataset.groupby(dataset.Timestamp)
    plt.figure(figsize=(10, 10))
    group.Text.size().plot.barh()
    plt.xticks(rotation=0)
    plt.savefig(f'static/images/{filename}_tweet_daily.png',
...
def suggestions():
    ...
    if text:
        r = requests.get(
            'http://suggestqueries.google.com/complete/search?output=toolbar&hl=en&q={}&gl=in'.format(text))
